...
def convert_image_to_gyotaku(dt_now, path):
    ...
    cv2.imwrite(filepath, img_threshold)
...
def background_eliminate_deeplabv3_resnet101(dt_now, path):
    img = cv2.imread(path)
    ...
    filepath = os.path.join(environment.UPLOAD_FOLDER, "no_bg_dl_" + dt_now + ".png")
